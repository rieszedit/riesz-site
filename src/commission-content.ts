...
export const notesJa = [
  ...
  'ラフイラストの段階からレイアウト・モーション・合成を進める場合は、ラフ素材先行進行として +30,000円〜（税別）となります。同一構図・同一サイズ・同一ポーズ・同等のレイヤー構成での清書差し替え1回を含みます。',
...
export const notesEn = [
  ...
  'Starting layout, animation, and compositing from rough artwork is available from an additional JPY 30,000 before tax. This includes one replacement with final artwork that retains the same composition, dimensions, pose, and equivalent layer structure.',
